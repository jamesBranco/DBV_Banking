...
public class InMemoryStore {
    ...
    public Optional<UserRecord> findUserByEmail(String email) { return Optional.ofNullable(usersByEmail.get(email)); }
    public Optional<UserRecord> findUserById(String id) { return Optional.ofNullable(usersById.get(id)); }
    public void saveUser(UserRecord u) {
        usersById.put(u.id, u); usersByEmail.put(u.email, u);
    }

    public void saveAccount(Account a) { accountsById.put(a.getId(), a); }
    public Optional<Account> findAccountById(String id) { return Optional.ofNullable(accountsById.get(id)); }
    public List<Account> findAccountsByUserId(String userId) {
        List<Account> out = new ArrayList<>();
        for (Account a : accountsById.values()) if (a.getOwnerUserId().equals(userId)) out.add(a);
        return out;
    }
